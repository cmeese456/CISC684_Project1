...
def id3(examples_list, target_attribute, attributes_list):
    ...
    if all(labels == 1 for labels in list(examples_list[target_attribute])):
        root.label = 1
    # Elif all examples are negative, set root's label to "0"
    elif all(labels == 0 for labels in list(examples_list[target_attribute])):
        root.label = 0
    # Elif attributes_list is empty, set root's label to most common value of target attribute in examples
...
def variance_impurity_gain(s, attr):
    ...
    variance_gain = variance_s[1] - (
            (attr.getTotalVal0() / attr.getTotal()) * variance_attr_0[1] - (attr.getTotalVal1() / attr.getTotal()) *
            variance_attr_1[1])
...
def count_n_on_leaf(root):
    ...
    return 1 + count_n_on_leaf(root.left) + count_n_on_leaf(root.right)
